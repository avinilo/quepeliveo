...
// Forward requests from /api/tmdb/* to https://api.themoviedb.org/3/*
...
export default async function handler(req, res) {
  try {
    ...
    // Copiar parámetros de consulta, excepto api_key (lo sobreescribimos)
    Object.entries(req.query).forEach(([key, value]) => {
      if (key === 'path') return;
      if (key === 'api_key') return;
      if (Array.isArray(value)) {
        // set all values (append to support multiple)
    ...
    if (method !== 'GET' && method !== 'HEAD' && req.body) {
      try {
        ...
        // Si falla el stringify, omitimos body
      }
    }
