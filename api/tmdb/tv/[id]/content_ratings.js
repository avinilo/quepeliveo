...
export default async function handler(req, res) {
  try {
    ...
    const idMatch = urlPath.match(/\/api\/tmdb\/tv\/(\d+)\/content_ratings/);
    ...
    const targetUrl = new URL(`${TMDB_BASE}tv/${id}/content_ratings`);
    ...
    console.error('Error en proxy TMDb (tv content_ratings):', err);
    res.status(500).json({ error: 'Error interno en proxy TMDb (tv content_ratings)', details: String(err && err.message || err) });
  }
}