...
export default async function handler(req, res) {
  try {
    ...
    const idMatch = urlPath.match(/\/api\/tmdb\/tv\/(\d+)\/watch\/providers/);
    ...
    const targetUrl = new URL(`${TMDB_BASE}tv/${id}/watch/providers`);
    ...
    console.error('Error en proxy TMDb (tv watch/providers):', err);
    res.status(500).json({ error: 'Error interno en proxy TMDb (tv watch/providers)', details: String(err && err.message || err) });
  }
}