...
export default async function handler(req, res) {
  try {
    ...
    // Subruta explícita para movie/*
    const urlPath = (req.url || '').split('?')[0] || '';
    const rest = urlPath.replace(/^\/api\/tmdb\/movie\/?/, '');
    const targetUrl = new URL(TMDB_BASE + 'movie/' + rest);
    ...
    console.error('Error en proxy TMDb (movie):', err);
    res.status(500).json({ error: 'Error interno en proxy TMDb (movie)', details: String(err && err.message || err) });
  }
}