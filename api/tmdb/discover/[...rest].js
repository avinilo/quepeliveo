...
export default async function handler(req, res) {
  try {
    ...
    // Subruta explícita para discover/*
    const urlPath = (req.url || '').split('?')[0] || '';
    const rest = urlPath.replace(/^\/api\/tmdb\/discover\/?/, '');
    const targetUrl = new URL(TMDB_BASE + 'discover/' + rest);
    ...
    console.error('Error en proxy TMDb (discover):', err);
    res.status(500).json({ error: 'Error interno en proxy TMDb (discover)', details: String(err && err.message || err) });
  }
}